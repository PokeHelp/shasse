...
import {
    ...
    TypeName,
    TypesResponse,
    Pokedex as PokedexType, Type
} from "@types";
import {useState, useEffect, useMemo, JSX, ChangeEvent} from "react";
import {getFormChoice, getOwnedPokemonUser} from "@service";
...
export default function Livingdex({userId}: { userId: string }): JSX.Element
{
    const t: Translation = useTranslations();
    const [ownedPokemon, setOwnedPokemon] = useState<OwnedPokemonIdByUser[]>([]);
    const [searchOnlyOwned, setSearchOnlyOwned] = useState<boolean>(false)
    ...
    useEffect((): void =>
    {
        (async (): Promise<void> =>
        {
            setOwnedPokemon(await getOwnedPokemonUser(userId));
        })();
    }, [userId]);
    ...
    const filteredData: PokedexType[] = useMemo((): PokedexType[] =>
    {
        ...
        return allPokemon.filter((pokemon: PokedexType): boolean =>
        {
            ...
            const ownedMatch: boolean = searchOnlyOwned ? ownedPokemon.some((owned: OwnedPokemonIdByUser): boolean => pokemon.id === owned.pokemonId) : true;

            return nameMatch && typeMatch && genMatch && ownedMatch;
        });
    }, [data, search, typeNameSelected.id, generationSelected.id, searchOnlyOwned, ownedPokemon]);
                ...
                <div className="flex w-full xl:w-3/5 xl:justify-end">
                    <Collapse triggerAction={(isOpen: boolean): JSX.Element => (
                        ...
                        <div className="flex gap-2 flex-wrap gap-y-2 justify-center">
                            ...
                            <div className="flex items-center gap-1">
                                <CheckboxUi
                                    checked={searchOnlyOwned}
                                    onCheckedChange={(): void => setSearchOnlyOwned(!searchOnlyOwned)}
                                />
                                <Typography>Uniquement mes shiny</Typography>
                            </div>
                        </div>
                    </Collapse>
                </div>
            ...
            <div className="flex flex-wrap gap-4 gap-y-9 justify-center">
                {paginatedData.map((pokemon: PokedexType): JSX.Element => (
                    <PokedexCard key={pokemon.id} pokemon={pokemon}
                                 ...
                                 pictureStyle="HOME"
                                 pictureClassName={ownedPokemon.some((owned: OwnedPokemonIdByUser): boolean => owned.pokemonId === pokemon.id) ? "" : "invert-30 brightness-0"}
                                 showTypes={false}
