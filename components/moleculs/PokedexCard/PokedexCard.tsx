...
export default function PokedexCard({pokemon, ...other}: PokedexCardProps): JSX.Element
{
    ...
    return (
        <Card {...other} className="border border-primary w-fit p-0 relative gap-0 cursor-pointer">
            ...
            <Picture src={getPokemonPictureFromId(pokemon.international_number)}
                     alt={t('pokemonPictureAlt', {pokemonName: pokemon.name})}
                     width={250} height={250} className="pt-7 p-4" key={pokemon.id}
            />
            ...
            <CardFooter className="p-0 justify-around py-4">
                ...
                <div className="flex gap-2">
                    {pokemon.types.map((type): JSX.Element => (
                        <Picture src={getTypePictureById(type.type_id)} width={25} height={25} title={type.type_name}
                                 alt={t('typePictureAlt', {typeName: type.type_name})} key={type.type_id}
                        />
                    ))}
                </div>
            </CardFooter>
